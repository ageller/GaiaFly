...
function defineParams(){
	ParamsInit = function() {
		...
		this.sTeff = 5780.;
		//factor to exagerate color (set to 1 for no exaggeration)
		this.Teffac = 1.0;

		//if >0, use this to move the points linearly, given velocity
		// for now I will assume units here are pc/Myr ~ km/s, so dt in Myr for moving in units of parsecs
		this.dt = 0.;
		this.time = 0.;
		this.dtfac = 0.001; //factor to add/subtract to dt every time user presses up/down
...
function init() {
	...
	if (params.reset){
        ...
        //black body texture
        params.tex['bb'] = new THREE.TextureLoader().load( "src/textures/bb.png" );
        params.tex['bb'].minFilter = THREE.LinearFilter;
	}
...
function calcSizeLimits(p, key){
//calculate limits for the size
	
	var j=0;
	if (params.parts[p][key] != null){
		var i=0;
		...
		params.parts[p][key+"Max"] = max;
		params.parts[p][key+"Min"] = min;
