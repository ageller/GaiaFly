...
/////////////////////////////////////////////
// dTime slider 
function setDTSliderHandle(i, value, parent) {
	parent.noUiSlider.set(value);
	params.dt = parseFloat(value);
	mouseDown = false; 
}

// Listen to keydown events on the input field.
function handleDTSliderText(input, handle) 
{
	...
	input.addEventListener('keydown', function( e ) {
		...
		switch ( e.which ) {
			case 13:
				setDTSliderHandle(handle, this.value, input.parent);
				break;
			case 38:
				setDTSliderHandle(handle, value + step, input.parent);
				break;
			case 40:
				setDTSliderHandle(handle, value - step, input.parent);
				break;
...
function createDTslider(){

	params.SliderDT = document.getElementById('DTSlider');
	params.SliderDTmax = document.getElementById('DTMaxT');
	if (params.SliderDT != null && params.SliderDTmax != null){
		if (params.SliderDT.noUiSlider) {
			params.SliderDT.noUiSlider.destroy();
		}
		params.SliderDTInputs = [params.SliderDTmax];
		params.SliderDTInputs[0].parent = params.SliderDT;
		min = 0.;
		max = 1.;

		noUiSlider.create(params.SliderDT, {
			start: [params.dt],
			connect: [true, false],
			tooltips: false,
			steps: [0.001],
		...
		params.SliderDT.noUiSlider.on('mouseup', mouseDown=false); 
		params.SliderDT.noUiSlider.on('update', function(values, handle) {
			params.SliderDTInputs[handle].value = values[handle];
			params.dt = parseFloat(values[handle]);
			mouseDown = true;
		});

		params.SliderDTInputs.forEach(handleDTSliderText);
	}
	w = parseInt(d3.select("#DTSlider").style("width").slice(0,-2));
	d3.select("#DTSlider").select('.noUi-base').style('width',w-10+"px");
}
...
function updateUITimeText()
{
	document.getElementById("TimeText").value = params.time;
}
...
function checkText(input, event)
{
	...
	if (key==13){
		...
		if (input.id == "TimeText"){
			params.time = parseInt(input.value);
		}
	}
...
//find the position in the partsKeys list
	...
	if (i < params.partsKeys.length-1){
		...
		if (pID == "Camera"){
			c = document.getElementById("TimeDiv");
			pb = 5;
			if (params.gtoggle[pID]){
...
//change the hamburger to the X to start
	...
	//time
	params.gtoggle.Time = true;
	var t1 = UI.append('div')
		.attr('id','TimeDiv')
		.attr('class','particleDiv');
	t1.append('div')
		.attr('class','pLabelDiv')
